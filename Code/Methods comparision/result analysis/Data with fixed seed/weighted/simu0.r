...
# We first try to simulate the data by haplotype bank, the simulation data would give us the phenotype, genotype, haplotype and the similarity matrix for the haplotypes
...
simulation 		<- function(Sample_size,SNP_posi,Risk_Model, risk_effect){
	...
	SNP11_posi	 	<- SNP_posi[1]                                    	    #locate the 1st SNP in RBJ as the causal SNP
	SNP12_posi		<- SNP_posi[2]									#locate the last SNP in RBJ as the causal SNP
	SNP21_posi	 	<- SNP_posi[3]                                    	    #locate the 1st SNP in GPRC5B as the causal SNP
	SNP22_posi		<- SNP_posi[4]									#locate the last SNP in GPRC5B as the causal SNP
	...
	tempA 			<- round(runif(Sample_size,1,N_sample_RBJ))
	GeneA 			<- Gene_RBJ[tempA,]

	tempB 			<- round(runif(Sample_size,1,N_sample_GPRC5B))
