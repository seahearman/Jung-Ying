...
RealisticSimulation <- function(Sample_size,SNP_posi,Risk_Model, risk_effect){
  ...
  if (Risk_Model==6){
		Main_effect <- risk_effect*Causal_SNP[,3]
		Epi_effect  <- 0
	}
...
OneGeneTest = function(Y,X,Simi1,Simi2){
  ...
  cat("sigma=",sigma_new,"Tau_A=",Tau_A_new,"\n")
	P0 					  = P 
...
for (i in 1:N.iter){
  cat("round=",i,"\n")
  SData = RealisticSimulation(Sample_size=N,SNP_posi,Risk_Model=6, risk_effect=1)
  Y     = SData$Y
  X     = SData$X
  gene1 = SData$geno1
  gene2 = SData$geno2
...
cat(type.I.error,"\n")
cat("If G_1 not equal to 0, The type I error for Epi test (averageIBS) is ",type.I.error,"\n",file="OneGeneAV.txt")