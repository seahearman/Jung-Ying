...
p     = 0
for (i in 1:N.iter){
  SData = RealisticSimulation(Sample_size=N,SNP_posi,Risk_Model=1, risk_effect=2)
  Y     = SData$Y
  X     = SData$X
  gene1 = SData$geno1
  gene2 = SData$geno2
  ...
  pvalue    = EpiTest(Y,X,Simi1,Simi2)
  cat("round=",i,"\n")
  if (pvalue<0.05) p=p+1
}
type.I.error = p/N.iter
cat("The type I error for Epi test is ",type.I.error,"\n")
cat("This is the case G1 != 0, G2!=0 G12=0, to get the type I error with average IBS, the rate is",type.I.error,"\n", file="Epi3TY.txt")
