...
for (gA1 in 1:3){
  for (gB1 in 1:5){
    ...
    SNP_posi = rbind(SNP_posi,array(c(SA1,(SA1+1),SB1,(SB1+1)),dim=c(1,4)))
...
for (round in 1:NComb){
    ...
    for (i in 1:N.iter){
      ...
      #- simulate the data ------
      SData = RealisticSimulation(Sample_size=N,SNPs,Risk_Model=2, risk_effect=0.25)
      Y     = SData$Y
      X     = SData$X
      gene1 = SData$geno1
      gene2 = SData$geno2
...
write.table(final.output,"Model2.txt",sep=" ",row.name=F,col.name=F)
